...
class FragNetLayerA(nn.Module):
    ...
    def forward(self, x_atoms, 
        ...
        # message = torch.cat([target_features, source_features, ea_bonds], dim=-1)  
        ...
        new_fbond_features=None
        summed_attn_weights_frags=None
        summed_attn_weights_fbonds=None
        if self.return_attentions:
            return x_atoms_new, x_frags, new_bond_features, new_fbond_features, summed_attn_weights_atoms, summed_attn_weights_frags, summed_attn_weights_bonds, summed_attn_weights_fbonds
        else:
            return x_atoms_new, x_frags, new_bond_features, new_fbond_features
...
class FragNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        x_atoms, x_frags, edge_features, _ = self.layers[0](x_atoms, edge_index, edge_attr, 
                               x_frags, atom_to_frag_ids,
        ...
        for layer in self.layers[1:]:
            x_atoms, x_frags, edge_features, _ = layer(x_atoms, edge_index, edge_features, 
                                       x_frags, atom_to_frag_ids,
        ...
        return x_atoms, x_frags, edge_features, None
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x, batch_ids=None, return_attention=False, attn_bias=None):
        ...
        values = values.reshape(batch_size*seq_length, self.embed_dim)
        # torch.masked_select(values.reshape(-1, embed_dim), ~mask.flatten())
...
class TransformerEncoder(nn.Module):
    ...
    # def get_attention_maps(self, x, attn_bias=None):
    #     attention_maps = []
    #     for l in self.layers:
...
#         self.frag_transformer =  TransformerConv(in_channels=emb_dim, out_channels=emb_dim, heads=transformer_heads)
        ...
        # self.f1 = nn.Linear(emb_dim, 3*emb_dim)
