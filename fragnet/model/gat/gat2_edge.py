...
class FragNetLayerA(nn.Module):
    def __init__(self, atom_in=128, atom_out=128, frag_in=128, frag_out=128,
                 edge_in=128, edge_out=128, num_heads=2, bond_edge_in=1, return_attentions=False,
        ...
        self.cnx_attr_transform = nn.Linear(8, self.edge_out)
    ...
    def forward(self, x_atoms, 
        ...
        # node_feautures_bond_graph = self.bond_edge_embed(node_feautures_bond_graph)
        ...
        if self.add_frag_self_loops:
            frag_index, _ = add_self_loops(edge_index=frag_index)
        ...
        cnx_attr = self.cnx_attr_transform(cnx_attr)
        cnx_attr_repeat = cnx_attr.repeat(self.num_heads,1, 1).permute(1,0,2)

        message = torch.cat([target_features, cnx_attr_repeat, source_features], dim=-1)  
        ...
        summed_attn_weights_frags = scatter_add(attn_probs, source, dim=0) # total attention attributed to each node in the bond graph, or each edge in the atom graph 

        x_frags_new = node_feats_sum_f.view(num_nodes_f, -1) 
    
        if self.return_attentions:
            return x_atoms_new, x_frags_new, new_bond_features, summed_attn_weights_atoms, summed_attn_weights_frags, summed_attn_weights_bonds
        else:
            return x_atoms_new, x_frags_new, new_bond_features
...
class FragNet(nn.Module):
    ...
    def __init__(self, num_layer, drop_ratio = 0.2, emb_dim=128, 
                 atom_features=167, frag_features=167, edge_features=16, num_heads=4):
        super().__init__()
    ...
    def forward(self, batch):
        ...
        x_atoms, x_frags, edge_features = self.layers[0](x_atoms, edge_index, edge_attr, 
                               ...
                               node_feautures_bond_graph,edge_index_bonds_graph,edge_attr_bond_graph, cnx_attr)
        ...
        for layer in self.layers[1:]:
            x_atoms, x_frags, edge_features = layer(x_atoms, edge_index, edge_features, 
                                       frag_index, x_frags, atom_to_frag_ids,
                                      edge_features,edge_index_bonds_graph,edge_attr_bond_graph, cnx_attr)
            
            x_atoms, x_frags = self.act(self.dropout(x_atoms)), self.act(self.dropout(x_frags))
            edge_features = self.act(self.dropout(edge_features))
        

        return x_atoms, x_frags, edge_features
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x, batch_ids=None, return_attention=False, attn_bias=None):
        ...
        fin = pad_sequence(a, batch_first=True, padding_value = 1) # padding value can affect accuracy of the mask. for
        # example if the padding_value = 0, fin.sum(-1) might not become equal to 0 due to some reason.
