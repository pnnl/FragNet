...
def continuous_creation_add_new(args):
    ...
    for i in range(l_limit, h_limit, n_rows):
        ...
        ds = get_pt_dataset(
            dfi_rem, args.data_type, maxiters=args.maxiters, frag_type=args.frag_type
        )
        ds = extract_data(ds)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="pretrain_data/unimol/ds",
    )
    ...
    parser.add_argument(
        "--raw_data_path", help="", type=str, required=False, default=None
    )
    parser.add_argument(
        "--calc_type", help="", type=str, required=False, default="scratch"
    )
