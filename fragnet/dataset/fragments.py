...
def get_3Dcoords2(smiles, numconf=1, maxiters=200):
    ...
    try:
        o = AllChem.MMFFOptimizeMoleculeConfs(mol, numThreads=0, mmffVariant='MMFF94s', maxIters=maxiters)
    except:
        return None
...
class Fragment:
    def __init__(self, graph, atom_indices, FragIdx=0):
        ...
        frag_rdmol = Chem.MolFromSmiles(MolFragmentToSmiles(graph.mol, atom_indices ), sanitize=False)
...
class Connection:
    def __init__(self, frag1, frag2, bond_atom_id1, bond_atom_id2, bond_index, bond_type, bond):
...
class FragmentedMol:
    def __init__(self, mol, conf, frag_type='brics'):
        ...
        for frag_bond in frag_bonds:
            ...
            connection = Connection(frag1, frag2, atom_id1, atom_id2, bond_index, bond_type, bond)
            ...
            for j in range(i+1, len(sg_frags)):
                ...
                for fragi in set_i:
                    ...
                    for fragj in set_j:
                        ...
                        fragj_cnx_ids = []
                        for ic in fragi_cnx:
                            fragi_cnx_ids.append( sorted( ( ic.BeginFragIdx, ic.EndFragIdx) ) )
         
                        if sorted( (fragi.FragIdx, fragj.FragIdx)) not in fragi_cnx_ids:
                            # print('updating')
                            connection = Connection(fragi, fragj, None, None, None, 'iso_cn3', bond)
