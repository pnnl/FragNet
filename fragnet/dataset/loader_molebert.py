""" "
Codes containing in this script were obtained from the
repository https://github.com/junxia97/Mole-BERT and modified
"""
...
def _load_tox21_dataset(input_path):
    ...
    # convert 0 to -1
    # labels = labels.replace(0, -1) # gihan
    # convert nan to 0
...
def _load_clintox_dataset(input_path):
    ...
    labels = input_df[tasks]
    # convert 0 to -1
    # labels = labels.replace(0, -1) # won't convert 0 to -1 # gihan
    # there are no nans
    assert len(smiles_list) == len(preprocessed_rdkit_mol_objs_list)
    assert len(smiles_list) == len(preprocessed_smiles_list)
    assert len(smiles_list) == len(labels)
