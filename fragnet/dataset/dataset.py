...
def get_pt_dataset(df, data_type, maxiters=200, frag_type='brics'):
    ...
    create_data = CreateData(data_type=data_type, create_bond_graph_data=create_bond_graph_data, 
...
class FinetuneData:
    ...
    def __init__(self, target_name, data_type, **kwargs):
        ...
        self.create_data = CreateData(data_type=data_type, create_bond_graph_data=True, 
                                      add_dhangles=False)
...
class FinetuneDataDTA:
    ...
    def __init__(self, target_name, data_type):
            self.target = target_name
            self.data_type = data_type
            self.create_data = CreateDataDTA(data_type=data_type, create_bond_graph_data=True, 
                                        add_dhangles=False)
...
class FinetuneDataCDRP:
    ...
    def __init__(self, target_name, data_type, args, use_genes=False):
        ...
        self.create_data = CreateDataCDRP(data_type=data_type, create_bond_graph_data=True, 
                                    add_dhangles=False)
    ...
    def load_datasets(self, opt):
        ...
        for k,v in opt.pretrain.datasets.items():
            if 'train' in v:
                ...
                train_dataset = self.remove_duplicates_and_add(train_dataset,path,name, include=include) 
            if 'val' in v:
                path = v.val.path
                name = v.val.name
                include=None
                if 'include' in v.val:
                    ...
                    include = range(include[0], include[1])
                val_dataset = self.remove_duplicates_and_add(val_dataset,path,name, include=include) 
            if 'test' in v:
                path = v.test.path
                name = v.test.name
                include=None
                if 'include' in v.test:
                    ...
                    include = range(include[0], include[1])
                test_dataset = self.remove_duplicates_and_add(test_dataset,path,name, include=include) 
