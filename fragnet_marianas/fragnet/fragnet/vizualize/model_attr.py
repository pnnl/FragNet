...
import matplotlib
from fragnet.vizualize.config import PROP_LIST, PROP_NAME_LIST
...
class FragNet(nn.Module):
    ...
    def __init__(self, num_layer, drop_ratio = 0.2, emb_dim=128, 
                ...
                apply_mask=False, mask_all_layers=False, bond_mask=None):
        super().__init__()
        ...
        self.mask_all_layers = mask_all_layers
...
#                                          )
        ...
        self.layers.append(FragNetLayerA(atom_in=atom_features, atom_out=emb_dim, frag_in=frag_features, 
                                   frag_out=emb_dim, edge_in=edge_features, fedge_in=fedge_in, 
                                   fbond_edge_in=fbond_edge_in, edge_out=emb_dim, num_heads=num_heads,
                                    bond_mask = bond_mask))
        ...
        for i in range(num_layer-1):
            self.layers.append(FragNetLayerA(atom_in=emb_dim, atom_out=emb_dim, frag_in=emb_dim, 
                                   frag_out=emb_dim, edge_in=emb_dim, edge_out=emb_dim, fedge_in=emb_dim,
                                             fbond_edge_in=fbond_edge_in, num_heads=num_heads, bond_mask=bond_mask))
    ...
    def forward(self, batch):
        ...
        if self.apply_mask and self.mask_all_layers:
            x_atoms[atom_mask==1] = 0.0 # maksing 
...
class FragNetFineTune(nn.Module):
    ...
    def __init__(self, n_classes=1, atom_features=167, frag_features=167, edge_features=17, 
                ...
                apply_mask=False, mask_all_layers=False):
        super().__init__()
        ...
        self.pretrain = FragNet(num_layer=num_layer, drop_ratio=drop_ratio, 
                               ...
                               apply_mask=apply_mask, mask_all_layers=mask_all_layers)
...
#-----------------------------------------
# Pretrain model. For energy calculations
#-----------------------------------------
...
def no_mask_predictions_property(dataset, chkpt_path, prop_type, args):
    ...
    if prop_type in PROP_LIST:
        ...
        if prop_type=='DRP':
            # model_no_mask = CDRPModel(model_no_mask, args.gene_dim, 'cpu')
            drug_model = FragNetFineTuneBaseViz(n_classes=args.finetune.model.n_classes, 
                                        ...
                                        drop_ratio=args.finetune.model.drop_ratio,
                                            ...
                                            act=args.finetune.model.act,
                                            fthead=args.finetune.model.fthead)
            model_no_mask = CDRPModel(drug_model, args.gene_dim, 'cpu')
    ...
    if prop_type =='DRP':
        test_loader = DataLoader(dataset, collate_fn=collate_fn_cdrp, batch_size=len(dataset), shuffle=False, drop_last=False)
    else:
        ...
        if prop_type=='DRP':
            drug_model = FragNetFineTuneBaseViz(n_classes=args.finetune.model.n_classes, 
                                        ...
                                        drop_ratio=args.finetune.model.drop_ratio,
                                            ...
                                            fthead=args.finetune.model.fthead,
                                            apply_mask=True)
            model_mask = CDRPModel(drug_model, args.gene_dim, 'cpu')

    elif prop_type == 'Energy':
        model_mask = FragNetPreTrain(num_layer=args.pretrain.num_layer, 
            drop_ratio=args.pretrain.drop_ratio,
                num_heads=args.pretrain.num_heads, 
                emb_dim=args.pretrain.emb_dim,
                atom_features=args.atom_features, 
...
def get_mol(smiles, remove_conf=True):
...
def highlight_atoms(atom_weights, mol):
    ...
    d2d = rdMolDraw2D.MolDraw2DSVG(width=WIDTH, height=HEIGHT)
...
def get_attr_image(smiles, model_config, chkpt_path, prop_type, gene_expr=None, **kwargs):
    ...
    if prop_type !='DRP':
        ...
        #TODO: Add frag_type
        cdrp_create_data = CreateDataCDRP(data_type='exp1s', create_bond_graph_data=True, 
                                            add_dhangles=False)
        
        dataset = cdrp_create_data.create_data_point(x)
        dataset = create_data([dataset])
    ...
    pred_no_mask, pred_mask, true = get_predictions(dataset, model_config, chkpt_path, prop_type)
    attribution = (pred_no_mask - pred_mask).numpy().ravel()
    atom_weights, frag_atoms = add_atom_weights(attribution, dataset)

    # print('attributions:', atom_weights)
    mol = get_mol(smiles)
